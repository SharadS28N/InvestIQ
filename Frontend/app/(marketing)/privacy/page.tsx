export default function PrivacyPage() {
  return (
    <div className="container py-12 max-w-4xl">
      <h1 className="text-3xl font-bold mb-6">Privacy Policy</h1>

      <div className="prose max-w-none">
        <p>Last updated: April 27, 2023</p>

        <h2 className="text-1xl font-bold">1. Introduction</h2>
        <p>
          At InvestIQ, we respect your privacy and are committed to protecting your personal data. This Privacy Policy
          explains how we collect, use, disclose, and safeguard your information when you use our platform.
        </p>
        <br />

        <h2 className="text-1xl font-bold">2. Information We Collect</h2>
        <p>We collect several types of information from and about users of our platform, including:</p>
        <ul>
          <li className="text-[15px]">Personal identifiers (name, email address, phone number)</li>
          <li className="text-[15px]">Account credentials</li>
          <li className="text-[15px]">Financial information (portfolio holdings, transaction history)</li>
          <li className="text-[15px]">Usage data (how you interact with our platform)</li>
          <li className="text-[15px]">Device information (IP address, browser type, operating system)</li>
        </ul>
<br />
        <h2 className="text-1xl font-bold">3. How We Collect Information</h2>
        <p>We collect information in the following ways:</p>
        <ul>
          <li>
            Direct collection: Information you provide when creating an account, updating your profile, or using our
            services
          </li>
          <li>Automated collection: Information collected automatically through cookies and similar technologies</li>
          <li>Third-party sources: We may receive information from third-party services connected to our platform</li>
        </ul>
      <br />
        <h2 className="text-1xl font-bold">4. How We Use Your Information</h2>
        <p>We use your information for various purposes, including:</p>
        <ul>
          <li className="text-[15px]">i) Providing and improving our services</li>
          <li className="text-[15px]">ii) Personalizing your experience</li>
          <li className="text-[15px]">iii) Processing transactions</li>
          <li className="text-[15px]">iv) Communicating with you</li>
          <li className="text-[15px]">v) Analyzing usage patterns</li>
          <li className="text-[15px]">vi) Training our AI models</li>
          <li className="text-[15px]">vii) Complying with legal obligations</li>
        </ul>
        <br />
        <h2 className="text-1xl font-bold">5. Data Security</h2>
        <p>
          We implement appropriate technical and organizational measures to protect your personal data against
          unauthorized or unlawful processing, accidental loss, destruction, or damage. However, no method of
          transmission over the Internet or electronic storage is 100% secure, and we cannot guarantee absolute
          security.
        </p>
        <br />
        <h2 className="text-1xl font-bold">6. Data Retention</h2>
        <p>
          We will retain your personal data only for as long as necessary to fulfill the purposes for which it was
          collected, including for the purposes of satisfying any legal, regulatory, tax, accounting, or reporting
          requirements.
        </p>
        <br />
        <h2 className="text-1xl font-bold">7. Your Rights</h2>
        <p>Depending on your location, you may have certain rights regarding your personal data, including:</p>
        <ul>
          <li className="text-[15px]">i) Right to access your personal data</li>
          <li className="text-[15px]">ii) Right to correct inaccurate data</li>
          <li className="text-[15px]">iii) Right to request deletion of your data</li>
          <li className="text-[15px]">iv) Right to restrict or object to processing</li>
          <li className="text-[15px]">v) Right to data portability</li>
          <li className="text-[15px]">vi) Right to withdraw consent</li>
        </ul>
        <br />
        <h2 className="text-1xl font-bold">8. Third-Party Services</h2>
        <p>
          Our platform may integrate with third-party services. This Privacy Policy does not apply to information
          collected by third parties, and we are not responsible for their privacy practices. We encourage you to review
          the privacy policies of any third-party services you access through our platform.
        </p>
        <br />
        <h2 className="text-1xl font-bold">9. Children's Privacy</h2>
        <p>
          Our platform is not intended for children under 18 years of age. We do not knowingly collect personal
          information from children under 18. If you are a parent or guardian and believe your child has provided us
          with personal information, please contact us.
        </p>
      <br />
        <h2 className="text-1xl font-bold">10. Changes to This Privacy Policy</h2>
        <p>
          We may update our Privacy Policy from time to time. We will notify you of any changes by posting the new
          Privacy Policy on this page and updating the "Last updated" date.
        </p>
<br />
        <h2 className="text-1xl font-bold">11. Contact Us</h2>
        <p>If you have any questions about this Privacy Policy, please contact us at privacy@investiq.com.np.</p>
      </div>
    </div>
  )
}
